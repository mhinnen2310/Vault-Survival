...
public class DialogService {
    ...
    private void openDistrictFarms(Player player,String status){DistrictData.District district=getDistrict(player);if(district==null){render(player,new DialogError("No District","Join a district first.").result());return;}var farms=plugin.getServiceRegistry().get(com.vaultsurvival.plugin.districts.DistrictFarmService.class);List<DialogMenuItem> items=new ArrayList<>();for(var farm:farms.farms(district.getId())){items.add(DialogMenuItem.status("#"+farm.id+" "+farm.name,"Type "+farm.type+" | level "+farm.level+" | workers "+farm.workerNpcIds.size(),farm.hasOutput()?"Output configured":"Output container required",farm.type==com.vaultsurvival.plugin.districts.DistrictFarmService.FarmType.CROPS?Material.WHEAT:Material.COW_SPAWN_EGG));items.add(DialogMenuItem.item("Set Output #"+farm.id,"Look at a nearby chest/barrel before clicking.","district farm output "+farm.id,null,Material.CHEST));items.add(DialogMenuItem.item("Place Worker #"+farm.id,"Worker must be within the configured farm distance.","district farm worker "+farm.id,null,Material.VILLAGER_SPAWN_EGG));if(farm.level<5)items.add(DialogMenuItem.item("Upgrade #"+farm.id,"Physical treasury cost: "+farms.nextUpgradeCost(farm),"district farm upgrade "+farm.id,null,Material.EXPERIENCE_BOTTLE));}items.add(DialogMenuItem.item("Create Crop Farm","Select an exact crop cuboid.","vsmenu input district_farm_crop",null,Material.WHEAT));items.add(DialogMenuItem.item("Create Animal Farm","Select an exact animal cuboid.","vsmenu input district_farm_animal",null,Material.COW_SPAWN_EGG));items.add(DialogMenuItem.item("Back to Town Clerk","Return to mayor controls.","vsmenu action district_town_clerk",null,Material.LECTERN));items.add(closeItem());render(player,new DialogResult(DialogResultType.NEXT,"District Farms",status,items));}
